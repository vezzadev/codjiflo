...
- The `codjiflo-action` GitHub Action (in-repo `action/`, published as `vezzadev/codjiflo-action`) posts a "Review in CodjiFlo" PR comment link; its hardcoded `codjiflo.vza.net` host moves to `codjiflo.net` (source + tests + rebuilt `dist/`). The standalone `github.com/vezzadev/codjiflo-action` repo must be synced from this source separately.
